...
import EventMenu from '../components/events/getEvents'; 
...
export default Events = ({navigation}) => {
  ...
  return (
    <View style={[styles.container, {backgroundColor: themeID.colorBackground}]}>
      ...
      {removeMenu ?
      <>
        <TouchableOpacity activeOpacity={1} style={[styles2.sortButton,{backgroundColor: themeID.colorButton1}]} onPress={()=> {setActionMenu(0);}}>
          <Image source={icons.checkmark} style={styles2.buttonIcon} />
        </TouchableOpacity>
        <TouchableOpacity activeOpacity={1} style={[styles2.deleteButton,{backgroundColor: themeID.colorButton1}]} onPress={()=> {setActionMenu(2)}} >
